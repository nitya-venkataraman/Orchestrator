#!/usr/bin/env python3
"""
Stage 4 (wireframe-ia / Figma Page Builder) Tier-1 validator — deterministic, no LLM.

Checks a page-builder artifact against `references/page-builder-reference.md`:
the base-role taxonomy, the page and component naming conventions, the required
UI states, the accessibility keys, the responsive coverage, and prototype
reachability.
...
This file and `ux-pipeline/validators/wireframe.py` MUST stay in sync: the
constants below are byte-identical in both, so the LangGraph gate and the
conversational skill enforce the same contract.

Usage:
    python scripts/validate_ds.py wireframe.json
    cat wireframe.json | python scripts/validate_ds.py -
